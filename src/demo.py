...
st.write(
    "Sube una o varias imágenes de tablas y visualiza cómo el pipeline las deskewea, "
    "detecta el texto, agrupa los cuadros y construye la tabla final."
)
...
for uploaded in uploaded_files:
    ...
    st.caption(f"Ángulo estimado: {result.angle_deg:.2f}° | Celdas: {result.n_rows}×{result.n_cols}")

    # Preprocesamiento
    st.subheader("1. Preprocesamiento")
    pre_cols = st.columns(4)
    pre_cols[0].image(image, caption="Original", use_column_width=True)
    pre_cols[1].image(ImageUtils.cv2_to_pil(result.deskewed), caption="Deskewed", use_column_width=True)
    ...
    st.image(ImageUtils.cv2_to_pil(clusters_img), caption="Asignación (fila, columna)", use_column_width=True)
