...
# Streamlit bootstrap expects to own the runtime, so we wrap all UI code into a
# function that is executed both when the module is imported by ``streamlit``
# and when it is invoked via ``python src/demo.py`` (see ``__main__`` guard
# below).  This helps newcomers who try to run the demo directly with Python
# and would otherwise face the "app keeps loading" behaviour.
...
def run_app() -> None:
    ...
    for uploaded in uploaded_files:
        ...
        pre_cols[0].image(image, caption="Original", use_column_width=True)
        pre_cols[1].image(
            ImageUtils.cv2_to_pil(result.deskewed), caption="Deskewed", use_column_width=True
        )
        ...
        if edges_img is not None:
            pre_cols[2].image(
                ImageUtils.cv2_to_pil(_ensure_color(edges_img)),
                caption="Canny / bordes",
        ...
        st.image(
            ImageUtils.cv2_to_pil(clusters_img),
            caption="Asignación (fila, columna)",
            use_column_width=True,
        )
...
if __name__ == "__main__":
    # Allow ``python src/demo.py`` to behave the same as ``streamlit run`` so
    # that new users do not get stuck wondering why the page never finishes
    # loading.  We delegate to Streamlit's CLI programmatically.
